...
def main():
    ...
    print("tttt")
    file_source = open(cns.FILE_PATH_SOURCE, "a")
    file_target = open(cns.FILE_PATH_TARGET, "a")
    ...
    while True:
        try:
            ...
            print(date1)
            auth = tweepy.OAuthHandler(cns.CONSUMER_KEY, cns.CONSUMER_SECRET)
            auth.set_access_token(cns.ACCESS_TOKEN, cns.ACCESS_TOKEN_SECRET)
            api = tweepy.API(auth)
            reply_stream_listener = ReplyStreamListener(api, file_source, file_target)
